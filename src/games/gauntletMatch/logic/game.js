...
export function playRound(roundNum, player, opp) {
  ...
  if (roundNum % 2 === 1) {
    ...
    const playerPinfallResult = attemptPinfall(
      player,
      opp,
      player.currentPinfall
    );
    events.push({
      type: 'pinfall',
      attacker: player.name,
      result: playerPinfallResult,
    });
...
export function executeMove(attacker, defender, move) {
  if (attacker.stamina < move.baseStaminaCost) {
    console.log(
      `${attacker.name} doesn't have enough stamina for ${move.name}`
    );
    return {
      type: 'move',
  ...
  const momentumGain =
    move.baseMomentum * (1 + 0.1 * attacker.getPlayerSkill(move.type));
  ...
  attacker.stamina = Math.max(
    0,
    Math.min(attacker.stamina + staminaChange, atkMaxStamina)
  );
...
export function attemptPinfall(attacker, defender, pinfall) {
  ...
  const lowHealthBoost =
    defender.health <= 20 ? ((20 - defender.health) / 20) * 0.5 : 0; // 0..0.5
  ...
  const baseChance =
    skillBonus * healthFactor * dirtyFactor + lowHealthBoost + zeroHealthBonus;
...
export function restoreStaminaAndHealth(player, opponent) {
  ...
  player.stamina = Math.min(
    player.stamina + player.getPlayerSkill(FighterTypes.LUCHADOR) * 2 + 40,
    pMaxStam
  );
  player.health = Math.min(
    player.health + player.getPlayerSkill(FighterTypes.SHOWMAN) * 2 + 10,
    pMaxHp
  );

  opponent.stamina = Math.min(
    opponent.stamina + opponent.getPlayerSkill(FighterTypes.LUCHADOR) * 2 + 40,
    oMaxStam
  );
  opponent.health = Math.min(
    opponent.health + opponent.getPlayerSkill(FighterTypes.SHOWMAN) * 2 + 10,
    oMaxHp
  );
}
