...
const GauntletMatch = () => {
  ...
  if (currentScreen === 'title') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-800 to-slate-900 flex flex-col items-center justify-center px-4">
        <h1 className="text-5xl sm:text-7xl font-bold text-white mb-6 sm:mb-8 drop-shadow-lg text-center">
          Gauntlet Match
        </h1>
        <p className="text-base sm:text-xl text-gray-300 mb-10 sm:mb-16">Made by Alon Barak</p>
        <button
          onClick={() => setCurrentScreen('fighterCreation')}
          className="px-8 sm:px-12 py-3 sm:py-4 text-xl sm:text-2xl font-bold text-slate-900 bg-white rounded-full border-4 border-white hover:bg-gray-100 hover:scale-105 transition-all duration-200 shadow-xl"
        >
          START GAME
        </button>
      </div>
  ...
  if (currentScreen === 'fighterCreation') {
    return (
      <div className="min-h-screen bg-gray-900 flex flex-col items-center justify-center px-4 sm:px-8 py-6">
        <div className="flex items-center justify-between w-full max-w-4xl mb-6 sm:mb-8">
          <h1 className="text-3xl sm:text-5xl font-bold text-white">Fighter Creation</h1>
        </div>

        <div className="w-full max-w-4xl bg-gray-800 rounded-lg p-4 sm:p-8 shadow-2xl">
          <div className="mb-8">
            <label className="text-2xl sm:text-3xl text-white mb-3 sm:mb-4 block">Name:</label>
            <input
              type="text"
              value={fighterName}
              onChange={(e) => setFighterName(e.target.value)}
              className="w-full px-3 sm:px-4 py-2 sm:py-3 text-lg sm:text-2xl bg-transparent border-2 border-cyan-400 rounded-lg text-white focus:outline-none focus:border-cyan-300"
            ...
            <div className="space-y-3 sm:space-y-4">
              {Object.entries(skillPoints).map(([skill, value]) => (
                ...
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                  <span className="text-xl sm:text-2xl text-white w-full sm:w-48">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className={`text-3xl sm:text-4xl font-bold w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded transition-colors ${
                ...
                <div className="text-xs text-gray-400 ml-1 sm:ml-2 mb-2">{skillDescriptions[skill]}</div>
                </React.Fragment>
              ))}
            </div>
          </div>
          ...
          <button
            ...
            className={`w-full py-3 sm:py-4 text-xl sm:text-2xl font-bold rounded-lg transition-all ${
  ...
  if (currentScreen === 'selectFighter') {
    ...
    // Debug: Log fighter data to console
    console.log('Fighter selection - available fighters:', fighters.map(f => ({
      name: f.name,
      img: f.img,
      hasImg: !!f.img
    })));
    ...
    return (
  <div key={renderTick} className="min-h-screen bg-gradient-to-br from-slate-800 to-slate-900 flex flex-col items-center justify-center px-4 sm:px-8 py-6">
        <h1 className="text-3xl sm:text-5xl font-bold text-white mb-6 sm:mb-12 text-center">SELECT FIGHTER</h1>
        
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 sm:gap-6 max-w-md sm:max-w-4xl">
      ...
      const specialtyType = Object.values(FighterTypeMeta).find(m => m.name === specialtyName) || null;
            ...
            return (
              <button
                ...
                disabled={isDefeated}
                  className={`relative w-36 h-44 sm:w-48 sm:h-56 border-2 ${specialtyType ? specialtyType.border : 'border-cyan-400'} rounded-lg flex flex-col items-center justify-start pt-2 gap-1 transition-all ${
                  isDefeated
                ...
                {isDefeated ? (
                  <>
                    <div className="absolute inset-0 flex items-center justify-center">
                      <X className="w-20 h-20 sm:w-32 sm:h-32 text-cyan-400" strokeWidth={3} />
                    </div>
                    <span className="absolute text-red-500 font-bold text-2xl sm:text-4xl transform rotate-12">
                      DEFEATED
                    </span>
                  </>
                ) : (
                  <div className="text-center px-2 flex flex-col items-center">
                    {fighter.img ? (
                      <img 
                        src={fighter.img} 
                        alt={fighter.name} 
                        className="w-16 h-16 sm:w-24 sm:h-24 object-contain rounded-full border-2 border-white shadow mb-1" 
                        onError={(e) => {
                          console.error('Fighter selection image failed to load:', fighter.img, 'for fighter:', fighter.name);
                          e.target.src = GenericWrestlerImg;
                        }}
                        onLoad={() => {
                          console.log('Fighter selection image loaded successfully:', fighter.img);
                        }}
                      />
                    ) : (
                      <img 
                        src={GenericWrestlerImg} 
                        alt={fighter.name} 
                        className="w-16 h-16 sm:w-24 sm:h-24 object-contain rounded-full border-2 border-white shadow mb-1"
                        onError={(e) => {
                          console.error('Generic wrestler image failed to load:', GenericWrestlerImg);
                          e.target.style.display = 'none';
                        }}
                      />
                    )}
                    <div className="text-white w-full">
                      <div className="font-bold text-xs sm:text-sm leading-tight truncate" title={fighter.name}>
                        {fighter.name}
                      </div>
                      <div className={`text-[10px] sm:text-xs font-semibold ${specialtyType ? specialtyType.text : 'text-cyan-300'}`}>
                        {specialtyName}
                      </div>
                    </div>
                  </div>
        ...
        <div className="mt-6 sm:mt-8 text-white text-lg sm:text-xl">
  ...
  if (currentScreen === 'match') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-800 to-slate-900 flex flex-col items-center justify-center px-4 sm:px-8 py-6">
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-12 mb-6 sm:mb-8 w-full">
          <div className="w-full sm:w-auto">
            <WrestlerSection fighter={playerFighter} />
          </div>
          <div className="flex flex-col items-center gap-2 sm:gap-4 mx-0 sm:mx-8 order-first sm:order-none">
            <div className="text-3xl sm:text-5xl font-extrabold text-cyan-400 select-none">
              VS
            </div>
            ...
            {/* Match Status */}
            {matchWinner && (
              <div className="text-2xl sm:text-3xl font-bold text-cyan-400 text-center">
            ...
            {!matchWinner && awaitingNextRound && !isProcessingRound && (
              <button
                ...
                className="px-4 sm:px-6 py-2 rounded-lg bg-cyan-400 text-gray-900 font-bold text-base sm:text-lg hover:bg-cyan-300 transition-colors"
              >
        ...
        <div className="w-full max-w-3xl bg-gray-900 rounded-lg p-3 sm:p-4 mt-2 shadow-lg">
          <h2 className="text-lg sm:text-xl font-bold text-cyan-300 mb-2">Match Log</h2>
          <div ref={logContainerRef} className="h-40 sm:h-48 overflow-y-auto bg-gray-800 rounded p-2 text-white text-sm sm:text-base font-mono leading-snug">
            {matchLog.length === 0 ? (
  ...
  if (currentScreen === 'cardSelect') {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-800 to-slate-900 flex flex-col items-center justify-center px-4 sm:px-8 py-6">
        <h1 className="text-2xl sm:text-3xl font-bold text-white mb-4 sm:mb-8 text-center">Select 2 cards to discard</h1>
        <div className="flex flex-wrap justify-center gap-3 sm:gap-6 mb-6 sm:mb-8">
          {drawnCards.map((move, idx) => (
            ...
            >
              <MoveCard move={move} />
              <div className="text-center mt-2 text-xs sm:text-sm text-white">
          ...
          {pinfallCard && (
            <div>
              ...
              <div className="text-center mt-2 text-[10px] sm:text-xs text-cyan-300 font-bold">Pinfall (Auto-Keep)</div>
            </div>
          )}
        </div>
        <button
          className={`px-6 sm:px-8 py-2 sm:py-3 rounded-lg text-lg sm:text-xl font-bold transition-all ${
  ...
  if (currentScreen === 'defeated') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 to-red-900 flex flex-col items-center justify-center px-4 py-6 text-center">
        <h1 className="text-4xl sm:text-6xl font-bold text-white mb-3 sm:mb-4">DEFEATED</h1>
        <p className="text-lg sm:text-2xl text-gray-300 mb-8 sm:mb-12">Your run has ended. Start a new gauntlet.</p>
        <button
          onClick={resetGame}
          className="px-8 sm:px-12 py-3 sm:py-4 bg-red-600 text-white font-bold text-lg sm:text-xl rounded-lg hover:bg-red-500 shadow-lg"
        >
          New Run
        </button>
      </div>
  ...
  if (currentScreen === 'championship') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-yellow-600 to-yellow-900 flex flex-col items-center justify-center px-4 py-6">
        <Trophy className="w-20 h-20 sm:w-32 sm:h-32 text-yellow-300 mb-6 sm:mb-8 animate-bounce" />
        <h1 className="text-4xl sm:text-7xl font-bold text-white mb-3 sm:mb-4 text-center">
          CHAMPION!
        </h1>
        <p className="text-xl sm:text-3xl text-yellow-100 mb-6 sm:mb-8 text-center">
          You have conquered the gauntlet!
        </p>
        <p className="text-lg sm:text-2xl text-yellow-200 mb-8 sm:mb-12 text-center">
          Fighter: {fighterName}
        </p>
        
        <button
          onClick={resetGame}
          className="px-8 sm:px-12 py-3 sm:py-4 bg-white text-yellow-900 font-bold text-lg sm:text-xl rounded-lg hover:bg-yellow-100 shadow-xl"
        >
          New Game
        </button>
      </div>
